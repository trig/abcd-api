...
class Response
{
     ...
     * @param array $headers
     */
    public function __construct(int $status = self::HTTP_OK, string $body = '', array $headers = [])
    {
     ...
     * @return Response
     */
    public function setJson(array $json = []): Response
    {
        $this->setBody(json_encode($json, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT));
    ...
    {
        if (headers_sent()) {
            throw new \RuntimeException('Headers was already been sent');
        }

        header(sprintf("HTTP/1.1 %d %s", $this->httpStatus, $this->httpStatusMessages[$this->httpStatus] ?? ''));

        foreach (array_unique($this->headers) as $hName => $hValue) {
            $hValue = implode('; ', (array)$hValue);
            header("{$hName}: ${hValue}");
        }

        if ($cLength = $this->getContentLength()) {
            $this->headers['Content-Length'] = $cLength;
        }
